...
class Products extends CI_Controller {
	...
	public function get_subcategories(){
		...
		$sub = $this->Common_model->get_records("subcategories","*",array('cid'=>$category_id,"status"=>0));
		
		$mainarray = array();
		if(!empty($sub)){
			foreach($sub as $tmp){
				$subarray['id'] = $tmp->scid;
				$subarray['value'] = $tmp->subcategory_name_english;
	...
	public function save(){
		...
		$values['hashtag'] = trim(strip_tags($this->input->post('hashtag')));
		$values['description_english'] = $this->input->post('description_english');
		$values['description_tamil'] = $this->input->post('description_tamil');
		$values['short_description_english'] = $this->input->post('short_description_english');
		$values['short_description_tamil'] = $this->input->post('short_description_tamil');
